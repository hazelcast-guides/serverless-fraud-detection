= Serverless Fraud Detection with AWS and Hazelcast {hazelcast-cloud}
:description: In this tutorial, you'll build a serverless solution for detecting fraudulent transactions using AWS Lambda, Hazelcast Viridian, and Node.js.
...
- The `ImportAirportsFn` Lambda function ingests the airport data into the {hazelcast-cloud} Serverless cluster so that it can be queried with the lowest latency. This function is triggered by uploads to the S3 bucket so that any new data is ingested to the {hazelcast-cloud} Serverless cluster.

- The `ValidateFn` Lambda function implements the fraud detection logic to validate the transactions. This function communicates with the {hazelcast-cloud} Serverless cluster to read airport data.

- The Amazon API Gateway exposes the `ValidateFn` Lambda function to a REST API resource at the `/validate` path.

- The {hazelcast-cloud} Serverless cluster stores a copy of the airport data so that the `ValidateFn` Lambda function can read it faster than if it were in the S3 bucket.
...
. Open the Hazelcast {hazelcast-cloud} console, click *Connect Your Application*, and download the keystore files from the *Advanced Setup* tab.
...
The code that ingests airport data from the S3 bucket into your {hazelcast-cloud} Serverless cluster uses the following resources in the `code/` directory:
...
. Open the Hazelcast {hazelcast-cloud} console, click *Connect Your Application*, and go to the *Advanced setup* tab. Leave this window open. You'll need some of these details in the next step.
...
. Open the Hazelcast {hazelcast-cloud} console and go to *Management Center* > *SQL Browser*.
+
...
- Ingest data into a {hazelcast-cloud} Serverless cluster from an S3 bucket.
...
Learn more about Hazelcast {hazelcast-cloud} Serverless:
